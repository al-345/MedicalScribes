...
x = np.array(data['eff'][0:9])
y = np.arange(0, 9)
x2 = np.zeros(9)

fig, ax1 = plt.subplots(nrows=1, figsize=(9, 6))

error_vals = [(round(1.96 * data['sd_eff'][i], 6), round(1.96 * data['sd_eff'][i], 6)) for i in range(0, 9)]

x2_labels = [f"{data['eff'][i]} ({data['ci_low'][i]}, {data['ci_upp'][i]})" for i in range(0, 9)]

xerr = np.array(error_vals).T
...
plt.yticks(range(0, 9), index[0:9])
